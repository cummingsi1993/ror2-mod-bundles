...
- **Dropshipping** (lunar) — **you can no longer open gold chests**, but for every **(player count / 2)** items your teammates collect, a **copy is drop-shipped to you**. Extra stacks divide the throttle (faster copies, up to mirroring everything teammates collect). A pure group-play item: **useless solo** (no teammates to skim from).
