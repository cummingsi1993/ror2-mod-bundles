#!/bin/bash
# Retry batch for the remaining items at resolution 512 — the 1024 voxel volume of
# large solid shapes (ledger) blew past the WSL2 memory ceiling during mesh extraction.
set -u
...
for item in accounts_receivable golden_parachute final_notice; do
...
try:
  ...
  cp "$OUT/$fname" "$RAW/${item}_raw.glb" && echo "saved $RAW/${item}_raw.glb $(date +%H:%M:%S)"
done
