...
- **Fire Drill** equipment (`Items/FireDrill.cs`): `EquipmentSlot.PerformEquipmentAction` hook toggles a window (cancel if active, else start). `appearsInSinglePlayer=false`. **Easter egg:** used with the artifact *off* (no real emergency), it backfires — a `DamageType.NonLethal | BypassArmor` self-hit for `BackfireHealthFraction` (0.8) of max HP. Non-lethal so it can't drop you below 1. Deliberately undocumented in the item description and the Thunderstore README. Config `FireDrill.BackfireEnabled` / `BackfireHealthFraction`.
